  ...
  return (
    <section aria-label="TMDB catalog" className="mt-8">
      <div className="mb-6 flex flex-wrap gap-2">
        <button type="button" onClick={() => setGenre("")} className={`rounded-full border px-4 py-2 text-sm ${!genre ? "bg-primary text-primary-foreground" : "hover:bg-muted"}`}>Popular</button>
        {genres.map((item) => <button key={item.id} type="button" onClick={() => setGenre(String(item.id))} className={`rounded-full border px-4 py-2 text-sm ${genre === String(item.id) ? "bg-primary text-primary-foreground" : "hover:bg-muted"}`}>{item.name}</button>)}
      </div>
