  ...
  return (
    <div className="flex w-full flex-col gap-3">
      <div ref={playerRef} className="relative aspect-video min-h-[240px] w-full overflow-hidden bg-black">
        ...
        {type !== "movie" && (
          <Button type="button" variant="secondary" size="sm" className="absolute right-16 top-3 gap-2 bg-background/90 shadow-lg backdrop-blur-sm hover:bg-background" aria-expanded={drawerOpen} aria-controls="episode-drawer" onClick={() => setDrawerOpen((open) => !open)}>
      ...
      {type === "tv" && drawerOpen && (
        <aside id="episode-drawer" className="rounded-xl border border-border bg-card p-4 shadow-xl" aria-label="Season and episode selector">
          <div className="flex items-center justify-between gap-3">
            ...
            <Button type="button" variant="ghost" size="icon" onClick={() => setDrawerOpen(false)} aria-label="Close episode selector"><X /></Button>
          </div>
          <div className="mt-4 flex flex-wrap items-end gap-3">
            ...
            <Button type="button" onClick={() => selectEpisode(season, episode)} className="h-9 gap-2"><RotateCcw data-icon="inline-start" />Load episode</Button>
          </div>
        </aside>
      )}
    </div>
