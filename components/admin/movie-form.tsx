...
import { TmdbMetadataPicker } from "@/components/admin/tmdb-metadata-picker"
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="max-h-[90vh] overflow-y-auto sm:max-w-lg">
        ...
        <form onSubmit={onSubmit} className="space-y-4">
          <TmdbMetadataPicker type={movie?.isSeries ? "tv" : "movie"} onApply={(metadata) => setForm((current) => ({ ...current, title: metadata.title, description: metadata.description, thumbnailUrl: metadata.thumbnailUrl, year: metadata.year ? Number(metadata.year) : null, duration: metadata.duration, rating: metadata.rating, tmdbId: metadata.tmdbId }))} />
          <div className="space-y-2">
            <Label htmlFor="title">Title</Label>
            <Input id="title" value={form.title} onChange={(e) => set("title", e.target.value)} required />
          </div>

          <MediaUpload kind="image" label="Thumbnail image" value={form.thumbnailUrl} onChange={(url) => set("thumbnailUrl", url)} />
