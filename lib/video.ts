...
export function resolveVideoSource(url: string): VideoSource {
  ...
  // SuperEmbed / TMDB entries: use `tmdb:12345` or a bare TMDB ID.
  const superEmbedId = trimmed.match(/^(?:tmdb:)?(\d+)$/i)
  if (superEmbedId) {
    return {
      kind: "iframe",
      embedUrl: `https://getsuperembed.link/?video_id=${superEmbedId[1]}&tmdb=1`,
    }
  }
